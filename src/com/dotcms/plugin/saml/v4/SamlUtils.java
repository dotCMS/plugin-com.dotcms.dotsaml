...
public class SamlUtils
{
	...
	public static LogoutRequest buildLogoutRequest( final IdpConfig idpConfig, final NameID nameID, final String sessionIndexValue )
	{
	...
	public static AuthnRequest buildAuthnRequest( final HttpServletRequest request, final IdpConfig idpConfig )
	{
		...
		authnRequest.setProtocolBinding( idpConfig.getOptionString( DotSamlConstants.DOTCMS_SAML_PROTOCOL_BINDING, SAMLConstants.SAML2_ARTIFACT_BINDING_URI ) );
		...
		authnRequest.setForceAuthn( idpConfig.getOptionBoolean( DotSamlConstants.DOTCMS_SAML_FORCE_AUTHN, false ) );
	...
	public static String getIPDSSODestination( final IdpConfig idpConfig )
	{
		...
		return ( null != redirectIdentityProviderDestinationSSOURL ) ? redirectIdentityProviderDestinationSSOURL : idpConfig.getOptionString( DotSamlConstants.DOTCMS_SAML_IDENTITY_PROVIDER_DESTINATION_SSO_URL, null );
	}
	...
	public static String getIPDSLODestination( final IdpConfig idpConfig )
	{
		...
		// first check the meta data info., secondly the idpConfig
		return ( null != redirectIdentityProviderDestinationSLOURL ) ? redirectIdentityProviderDestinationSLOURL : idpConfig.getOptionString( DotSamlConstants.DOTCMS_SAML_IDENTITY_PROVIDER_DESTINATION_SLO_URL, null );
	}
	...
	{
		return idpConfig.getOptionString( DotSamlConstants.DOTCMS_SAML_SERVICE_PROVIDER_ISSUER, getSiteName( idpConfig ) );
	}
	...
	private static String getSiteName( final IdpConfig idpConfig )
	{
		...
		if ( UtilMethods.isSet( defaultHost ) )
		{
			...
			return !( defaultHost.trim().startsWith( DotSamlConstants.HTTP_SCHEMA ) || defaultHost.trim().startsWith( DotSamlConstants.HTTPS_SCHEMA ) ) ? DotSamlConstants.HTTPS_SCHEMA + defaultHost : defaultHost;
		}

		return idpConfig.getOptionString( DOT_SAML_DEFAULT_SERVICE_PROVIDER_PROTOCOL, DotSamlConstants.HTTPS_SCHEMA ) + "://" + SPIIssuerResolver.getDefaultServiceProviderIssuer().getHostname();
	}
	...
	public static NameIDPolicy buildNameIdPolicy( final IdpConfig idpConfig )
	{
		...
		nameIDPolicy.setAllowCreate( idpConfig.getOptionBoolean( DotSamlConstants.DOTCMS_SAML_POLICY_ALLOW_CREATE, false ) );
		...
		nameIDPolicy.setFormat( idpConfig.getOptionString( DotSamlConstants.DOTCMS_SAML_NAME_ID_POLICY_FORMAT, NameIDType.PERSISTENT ) );
	...
	public static RequestedAuthnContext buildRequestedAuthnContext( final IdpConfig idpConfig )
	{
		...
		passwordAuthnContextClassRef.setAuthnContextClassRef( idpConfig.getOptionString( DotSamlConstants.DOTCMS_SAML_AUTHN_CONTEXT_CLASS_REF, AuthnContext.PASSWORD_AUTHN_CTX ) );
	 ...
	 */
	public static AuthnContextComparisonTypeEnumeration getAuthnContextComparisonTypeEnumeration( final IdpConfig idpConfig )
	{
		AuthnContextComparisonTypeEnumeration comparisonTypeEnumeration = AuthnContextComparisonTypeEnumeration.MINIMUM;

		final String enumName = idpConfig.getOptionString( DotSamlConstants.DOTCMS_SAML_AUTHN_COMPARISON_TYPE, null );
	...
	public static Assertion getAssertion( final ArtifactResponse artifactResponse, final IdpConfig idpConfig )
	{
		...
		final Assertion assertion = decryptAssertion( encryptedAssertion, idpConfig ); /// this is the user message itself
	...
	public static Assertion getAssertion( final Response response, final IdpConfig idpConfig )
	{
		...
		if ( idpConfig.getOptionBoolean( DotSamlConstants.DOTCMS_SAML_IS_ASSERTION_ENCRYPTED, true ) )
		{
	...
	{
		Assertion assertion = null;
		final StaticKeyInfoCredentialResolver keyInfoCredentialResolver = new StaticKeyInfoCredentialResolver( getCredential( idpConfig ) );
	...
	public static void verifyAssertionSignature( final Assertion assertion, final IdpConfig idpConfig )
	{
		...
		if ( idpConfig.getOptionBoolean( DotSamlConstants.DOTCMS_SAML_CHECKIF_ASSERTION_SIGNED, true ) && !assertion.isSigned() )
		{
	...
	public static Credential createCredential( final IdpConfig idpConfig )
	{
		...
		try
		{
			...
			{
				keyStorePath = idpConfig.getOptionString( DotSamlConstants.DOTCMS_SAML_KEY_STORE_PATH, "" );
				password = idpConfig.getOptionString( DotSamlConstants.DOTCMS_SAML_KEY_STORE_PASSWORD, "" );
				keyEntryId = idpConfig.getOptionString( DotSamlConstants.DOTCMS_SAML_KEY_ENTRY_ID, DotSamlConstants.DOTCMS_SAML_KEY_ENTRY_ID_DEFAULT_VALUE );
				keyStoreEntryPassword = idpConfig.getOptionString( DotSamlConstants.DOTCMS_SAML_KEY_STORE_ENTRY_PASSWORD, password );
				...
				final String keyStoreType = idpConfig.getOptionString( DotSamlConstants.DOTCMS_SAML_KEY_STORE_TYPE, KeyStore.getDefaultType() );
	...
	public static Credential getCredential( final IdpConfig idpConfig )
	{
		if ( !credentialMap.containsKey( idpConfig.getSpEndpointHostname() ) )
		{
			...
			{
				Logger.error( SamlUtils.class, "The credential is null for the site: " + idpConfig.getSpEndpointHostname() );
	 ...
	 * @param keys Required Property keys required to be in the samlProperties.
	 * @return
	 */
