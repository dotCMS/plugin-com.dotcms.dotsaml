...
import com.dotcms.plugin.saml.v4.SiteIdpConfigResolver;
import com.dotcms.plugin.saml.v4.exception.AttributesNotFoundException;
import com.dotcms.plugin.saml.v4.exception.DotSamlException;
import com.dotcms.plugin.saml.v4.exception.NotNullEmailAllowedException;
import com.dotcms.plugin.saml.v4.exception.SamlUnauthorizedException;
...
import static com.dotcms.plugin.saml.v4.DotSamlConstants.*;
...
public class OpenSamlAuthenticationServiceImpl implements SamlAuthenticationService
{
	...
	@Override
	public void authentication( final HttpServletRequest request, final HttpServletResponse response, final String siteName )
	{
		...
		endpointContext.setEndpoint( getIdentityProviderDestinationEndpoint( configuration ) );
	...
	@SuppressWarnings( { "rawtypes", "unchecked" } )
	public void logout( final HttpServletRequest request, final HttpServletResponse response, final NameID nameID, final String sessionIndexValue, final String siteName )
	{
		...
		endpointContext.setEndpoint( getIdentityProviderSLODestinationEndpoint( configuration ) );
	 ...
	 */
	@Override
	public User getUser( final HttpServletRequest request, final HttpServletResponse response, final String siteName )
	{
		User user = null;
	 ...
	 * @param response
	 *            {@link HttpServletResponse}
	 * @param loginHttpSession
	 *            {@link HttpSession} session to store the
	 * @return User
	 */
	@Override
	public User getUser( final HttpServletRequest request, final HttpServletResponse response, final HttpSession loginHttpSession, final String siteName )
	{
		User user = null;
		final Assertion assertion;
		final SiteConfigurationResolver resolver = (SiteConfigurationResolver) InstancePool.get( SiteConfigurationResolver.class.getName() );
		...
		if ( this.isValidSamlRequest( request, response, siteName ) )
		{
			...
			if ( null != loginHttpSession && null != user && null != assertion )
			{
				...
				if ( null != samlSessionIndex )
				{
					...
					Logger.debug( this, "Already set the session index with key:" + ( configuration.getSiteName() + SAML_SESSION_INDEX ) + " and value" + loginHttpSession.getAttribute( configuration.getSiteName() + SAML_SESSION_INDEX ) );
					Logger.debug( this, "Already set the name id with key:" + ( configuration.getSiteName() + SAML_NAME_ID ) + " and value" + loginHttpSession.getAttribute( configuration.getSiteName() + SAML_NAME_ID ) );
				}
	...
	{
		final String emailField = configuration.getStringProperty( DOT_SAML_EMAIL_ATTRIBUTE, "mail" );
		final String firstNameField = configuration.getStringProperty( DOT_SAML_FIRSTNAME_ATTRIBUTE, "givenName" );
		final String lastNameField = configuration.getStringProperty( DOT_SAML_LASTNAME_ATTRIBUTE, "sn" );
		final String rolesField = configuration.getStringProperty( DOT_SAML_ROLES_ATTRIBUTE, "authorizations" );
		final String firstNameForNullValue = configuration.getStringProperty( DOT_SAML_FIRSTNAME_ATTRIBUTE_NULL_VALUE, null );
		final String lastNameForNullValue = configuration.getStringProperty( DOT_SAML_LASTNAME_ATTRIBUTE_NULL_VALUE, null );
		final boolean allowNullEmail = configuration.getBooleanProperty( DOT_SAML_EMAIL_ATTRIBUTE_ALLOW_NULL, true );
		...
		assertion.getAttributeStatements().forEach( attributeStatement ->
		{
			...
			attributeStatement.getAttributes().forEach( attribute ->
			{
				...
				else
				{
					...
					Logger.warn( this, attributeName + " attribute did not match any user property on the configuration: " + customConfiguration );
				}
			} );
		} );
	...
	{
		final String buildRolesStrategy = configuration.getStringProperty( DOTCMS_SAML_BUILD_ROLES, DOTCMS_SAML_BUILD_ROLES_ALL_VALUE );
	...
	private void addRoles( final User user, final AttributesBean attributesBean, final Configuration configuration )
	{
	...
	private void handleRoles( final User user, final AttributesBean attributesBean, final Configuration configuration, final String buildRolesStrategy ) throws DotDataException
	{
	...
	private void addRolesFromIDP( final User user, final AttributesBean attributesBean, final Configuration configuration, final String buildRolesStrategy ) throws DotDataException
	{
		String role = null;
		...
		{
			final String removeRolePrefix = configuration.getStringProperty( DOT_SAML_REMOVE_ROLES_PREFIX, StringUtils.EMPTY );
			final String[] rolePatterns = configuration.getStringArray( DOTCMS_SAML_INCLUDE_ROLES_PATTERN, null );
	...
	private void setSignatureSigningParams( final MessageContext context, final Configuration configuration )
	{
		...
		signatureSigningParameters.setSigningCredential( getCredential( configuration ) );
