...
public class SamlUtils
{
	...
	public static LogoutRequest buildLogoutRequest( final Configuration configuration, final NameID nameID, final String sessionIndexValue )
	{
		...
		if ( !isSet( idpSingleLogoutDestionation ) )
		{
			...
			throw new DotSamlException( "The property: " + DotSamlConstants.DOTCMS_SAML_IDENTITY_PROVIDER_DESTINATION_SLO_URL + " must be set on the host" );
		}
	...
	public static AuthnRequest buildAuthnRequest( final HttpServletRequest request, final Configuration configuration )
	{
		...
		if ( !isSet( ipDSSODestination ) )
		{
			...
			throw new DotSamlException( "The property: " + DotSamlConstants.DOTCMS_SAML_IDENTITY_PROVIDER_DESTINATION_SSO_URL + " must be set on the host" );
		}
		...
		authnRequest.setProtocolBinding( configuration.getStringProperty( DotSamlConstants.DOTCMS_SAML_PROTOCOL_BINDING, SAMLConstants.SAML2_ARTIFACT_BINDING_URI ) );
		...
		authnRequest.setForceAuthn( configuration.getBooleanProperty( DotSamlConstants.DOTCMS_SAML_FORCE_AUTHN, false ) );
	...
	public static String getIPDSSODestination( final Configuration configuration )
	{
		...
		return ( null != redirectIdentityProviderDestinationSSOURL ) ? redirectIdentityProviderDestinationSSOURL : configuration.getStringProperty( DotSamlConstants.DOTCMS_SAML_IDENTITY_PROVIDER_DESTINATION_SSO_URL, null );
	}
	...
	{
		final String redirectIdentityProviderDestinationSLOURL = configuration.getIdentityProviderDestinationSLOURL( configuration );

		// first check the meta data info., secondly the configuration
		return ( null != redirectIdentityProviderDestinationSLOURL ) ? redirectIdentityProviderDestinationSLOURL : configuration.getStringProperty( DotSamlConstants.DOTCMS_SAML_IDENTITY_PROVIDER_DESTINATION_SLO_URL, null );
	}

	public static String getAssertionConsumerEndpoint( final HttpServletRequest request, final Configuration configuration )
	{
		final String assertionConsumerEndpoint = configuration.getAssertionConsumerEndpoint();
	...
	{
		return configuration.getStringProperty( DotSamlConstants.DOTCMS_SAML_SERVICE_PROVIDER_ISSUER, getSiteName( configuration ) );
	}
	...
	private static String getSiteName( final Configuration configuration )
	{
		...
		if ( UtilMethods.isSet( defaultHost ) )
		{
			...
			return !( defaultHost.trim().startsWith( Configuration.HTTP_SCHEMA ) || defaultHost.trim().startsWith( Configuration.HTTPS_SCHEMA ) ) ? Configuration.HTTPS_SCHEMA + defaultHost : defaultHost;
		}

		return configuration.getStringProperty( DOT_SAML_DEFAULT_SERVICE_PROVIDER_PROTOCOL, Configuration.HTTPS_SCHEMA ) + "://" + SPIIssuerResolver.getDefaultServiceProviderIssuer().getHostname();
	}
	...
	public static NameIDPolicy buildNameIdPolicy( final Configuration configuration )
	{
		...
		nameIDPolicy.setFormat( configuration.getStringProperty( DotSamlConstants.DOTCMS_SAML_NAME_ID_POLICY_FORMAT, NameIDType.PERSISTENT ) );
	...
	public static RequestedAuthnContext buildRequestedAuthnContext( final Configuration configuration )
	{
		...
		requestedAuthnContext.setComparison( getAuthnContextComparisonTypeEnumeration( configuration ) );

		final AuthnContextClassRef passwordAuthnContextClassRef = buildSAMLObject( AuthnContextClassRef.class );

		passwordAuthnContextClassRef.setAuthnContextClassRef( configuration.getStringProperty( DotSamlConstants.DOTCMS_SAML_AUTHN_CONTEXT_CLASS_REF, AuthnContext.PASSWORD_AUTHN_CTX ) );
	...
	public static AuthnContextComparisonTypeEnumeration getAuthnContextComparisonTypeEnumeration( final Configuration configuration )
	{
		...
		final String enumName = configuration.getStringProperty( DotSamlConstants.DOTCMS_SAML_AUTHN_COMPARISON_TYPE, null );
	 ...
	 * @return Assertion
	 */
	public static Assertion getAssertion( final ArtifactResponse artifactResponse, final Configuration configuration )
	{
	 ...
	 * @return Assertion
	 */
	public static Assertion getAssertion( final Response response, final Configuration configuration )
	{
	 ...
	 * @return Assertion
	 */
	public static Assertion decryptAssertion( final EncryptedAssertion encryptedAssertion, final Configuration configuration )
	{
		Assertion assertion = null;
		final StaticKeyInfoCredentialResolver keyInfoCredentialResolver = new StaticKeyInfoCredentialResolver( getCredential( configuration ) );
	...
	private static void validateSignature( final Assertion assertion, final Collection<Credential> credentials ) throws SignatureException
	{
	 ...
	 *            {@link Assertion}
	 */
	public static void verifyAssertionSignature( final Assertion assertion, final Configuration configuration )
	{
	...
	public static Credential createCredential( final Configuration configuration )
	{
		...
		final CredentialProvider customCredentialProvider = configuration.getServiceProviderCustomCredentialProvider();
		...
		try
		{
			...
			{
				keyStorePath = configuration.getStringProperty( DotSamlConstants.DOTCMS_SAML_KEY_STORE_PATH, "" );
				password = configuration.getStringProperty( DotSamlConstants.DOTCMS_SAML_KEY_STORE_PASSWORD, "" );
				keyEntryId = configuration.getStringProperty( DotSamlConstants.DOTCMS_SAML_KEY_ENTRY_ID, DotSamlConstants.DOTCMS_SAML_KEY_ENTRY_ID_DEFAULT_VALUE );
				keyStoreEntryPassword = configuration.getStringProperty( DotSamlConstants.DOTCMS_SAML_KEY_STORE_ENTRY_PASSWORD, password );
				...
				final String keyStoreType = configuration.getStringProperty( DotSamlConstants.DOTCMS_SAML_KEY_STORE_TYPE, KeyStore.getDefaultType() );
	...
	public static Credential getCredential( final Configuration configuration )
	{
		if ( !credentialMap.containsKey( configuration.getSiteName() ) )
		{
			...
			{
				Logger.error( SamlUtils.class, "The credential is null for the site: " + configuration.getSiteName() );

				throw new DotSamlException( "The credential is null for the site: " + configuration.getSiteName() );
			}
	...
	private static Credential createIdpCredential( final Configuration configuration )
	{
		...
		final CredentialProvider customCredentialProvider = configuration.getIdProviderCustomCredentialProvider();
	...
	public static Credential getIdPCredentials( final Configuration configuration )
	{
		...
		{
			idpCredentialMap.put( configuration.getSiteName(), createIdpCredential( configuration ) );
		}
	...
	{
		final Element element = ( object instanceof SignableSAMLObject && SignableSAMLObject.class.cast( object ).isSigned() && object.getDOM() != null ) ? object.getDOM() : toElement( object );
	...
	public static Set<String> validateKeyStore( Properties samlProperties )
	{
		...
		final String keyEntryId = UtilMethods.isSet( samlProperties.getProperty( DotSamlConstants.DOTCMS_SAML_KEY_ENTRY_ID ) ) ? samlProperties.getProperty( DotSamlConstants.DOTCMS_SAML_KEY_ENTRY_ID ) : DotSamlConstants.DOTCMS_SAML_KEY_ENTRY_ID_DEFAULT_VALUE;
		final String keyStoreEntryPassword = ( UtilMethods.isSet( samlProperties.getProperty( DotSamlConstants.DOTCMS_SAML_KEY_STORE_ENTRY_PASSWORD ) ) ) ? samlProperties.getProperty( DotSamlConstants.DOTCMS_SAML_KEY_STORE_ENTRY_PASSWORD ) : keyStorePassword;
		Credential credential = null;
		...
		if ( pathToKeyStore != null && keyStorePassword != null && keyStoreType != null && keyEntryId != null && keyStoreEntryPassword != null )
		{
			...
			try
			{
				...
				if ( null == credential )
				{
					otherErrors.add( "Error reading Key Store, please check the: " + DotSamlConstants.DOTCMS_SAML_KEY_ENTRY_ID );
				}
	 ...
	 *            {@link Properties} (Key, Value) = (PropertyName, FilePath).
	 * @param keysRequired
	 *            Property keys required to be in the samlProperties.
	 * @return
	 */
