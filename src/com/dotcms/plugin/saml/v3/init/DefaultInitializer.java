...
public class DefaultInitializer implements Initializer {
    ...
    private void initScheduleUpdaterSAMLConfiguration() {
        ...
        final int initialDelay = Config.getIntProperty
                (DotSamlConstants.SCHEDULE_UPDATER_TASK_INITIAL_DELAY, 10); // starts on 10 seconds.
        final int delaySeconds = Config.getIntProperty
                (DotSamlConstants.SCHEDULE_UPDATER_TASK_DELAY_SECONDS, 10); // runs every 10 seconds.

        Logger.debug(this, "Starting the Scheduled Updater SAML Task, initialDelay: " + initialDelay +
                        ", delaySeconds = " + delaySeconds);

        // todo: on 4.x replace this by the DotInitScheduler.getScheduledThreadPoolExecutor
        this.getScheduledThreadPoolExecutor()
                        .scheduleWithFixedDelay(ScheduleUpdaterTask::reRunScheduleUpdaterTask, initialDelay, delaySeconds, TimeUnit.SECONDS);
    }
