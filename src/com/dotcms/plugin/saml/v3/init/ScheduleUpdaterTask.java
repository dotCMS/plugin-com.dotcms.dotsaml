    ...
    public void updateConfiguration () {
        ...
        try {
            ...
            final SiteCofigurationInitializerService siteCofigurationInitializerService =
                    (SiteCofigurationInitializerService) InstancePool.get(SiteCofigurationInitializerService.class.getName());
            final SiteConfigurationService siteConfigurationService =
                    (SiteConfigurationService)InstancePool.get
                            (SiteConfigurationService.class.getName());
            final HostService hostService =
                    (HostService) InstancePool.get(HostService.class.getName());

            final UpdatedHostResult updatedHostResult =
                    hostService.getUpdatedHosts();
            final List<Host>        updatedHosts     =
                    updatedHostResult.getUpdatedHosts();
            final List<String>        removedHosts     =
                    updatedHostResult.getRemovedHosts();
            ...
            if (null != removedHosts && removedHosts.size() > 0) {
                ...
                removedHosts.forEach(removeHost -> this.removeHostAndAlias
                        (removeHost, hostService, siteConfigurationService));
            }
            ...
            Logger.debug(this,
                    "Scheduled Updater SAML Task is already done, it took: " +
                            stopWatch.getTime() + " ms");
        }
    } // updateConfiguration.


    private void removeHostAndAlias (final String hostname,
                                     final HostService hostService,
                                     final SiteConfigurationService siteConfigurationService) {
        ...
        if (null != hostname) {
            ...
            hostService.getHostAlias(hostname).forEach
