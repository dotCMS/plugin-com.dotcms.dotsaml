...
/**
 * Default implementation, it is a wrapper of the Dot Cms
 * {@link com.dotmarketing.util.Config} In addition will determine if any
 * idp-metainfo.xml is there, if it is will create a Credential for each signing
...
public class DefaultDotCMSConfiguration implements Configuration
{
	...
	public DefaultDotCMSConfiguration( final SiteConfigurationBean siteConfigurationBean, final String siteName )
	{
		...
		this.descriptorParser = InstanceUtil.newInstance( this.getStringProperty( DotSamlConstants.DOT_SAML_IDP_METADATA_PARSER_CLASS_NAME, null ), DefaultMetaDescriptorServiceImpl.class );

		final String metaDescriptorResourcePath = this.getStringProperty( DotSamlConstants.DOTCMS_SAML_IDP_METADATA_PATH, null );

		Logger.debug( this, "For the sitename: " + siteName + ", the metaDescriptorResourcePath is " + metaDescriptorResourcePath );

		this.metadataBean = ( UtilMethods.isSet( metaDescriptorResourcePath ) ) ? this.getMetaData( metaDescriptorResourcePath ) : null;
	 ...
	 * 
	 * @param metaDescriptorResourcePath
	 *            {@link String}
	 * @return MetadataBean
	 */
	protected MetadataBean getMetaData( final String metaDescriptorResourcePath )
	{
		...
		try ( InputStream inputStream = InputStreamUtils.getInputStream( metaDescriptorResourcePath ) )
		{
	...
	{
		final String accessFilterValues = this.getStringProperty( DotSamlConstants.DOT_SAML_ACCESS_FILTER_VALUES, null );
	...
	{
		final String accessFilterValues = this.getStringProperty( DotSamlConstants.DOT_SAML_INCLUDE_PATH_VALUES, "^" + ASSERTION_CONSUMER_ENDPOINT_DOTSAML3SP + "$," + "^/dotCMS/login.*$," + "^/html/portal/login.*$," + "^/c/public/login.*$," + "^/c/portal_public/login.*$," + "^/c/portal/logout.*$," + "^/dotCMS/logout.*$," + "^/application/login/login.*$," + "^/dotAdmin.*$," + "^" + LOGOUT_SERVICE_ENDPOINT_DOTSAML3SP + "$" );
	...
	{
		final String logoutPathValues = this.getStringProperty( DotSamlConstants.DOT_SAML_LOGOUT_PATH_VALUES, "/c/portal/logout,/dotCMS/logout,/dotsaml/request/logout" );
	...
	{
		return ( null != this.metadataBean ) ? this.metadataBean.getCredentialSigningList() : null;
	}
	...
	public String getIdentityProviderDestinationSSOURL( final Configuration configuration )
	{
		...
		if ( null != this.metadataBean && null != this.metadataBean.getSingleSignOnBindingLocationMap() && this.metadataBean.getSingleSignOnBindingLocationMap().containsKey( bindingType ) )
		{
			url = this.metadataBean.getSingleSignOnBindingLocationMap().get( bindingType );
		}
	...
	public String getIdentityProviderDestinationSLOURL( final Configuration configuration )
	{
		...
		if ( null != this.metadataBean && null != this.metadataBean.getSingleLogoutBindingLocationMap() && this.metadataBean.getSingleLogoutBindingLocationMap().containsKey( bindingType ) )
		{

			url = this.metadataBean.getSingleLogoutBindingLocationMap().get( bindingType );
		}
