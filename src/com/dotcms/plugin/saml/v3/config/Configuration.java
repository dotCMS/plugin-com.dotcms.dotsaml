...
/**
 * The configuration encapsulates all the info necessary for the open saml
 * plugin Note: an implementation of {@link Configuration} must has a
 * constructor with {@link SiteConfigurationBean} arguments and a {@link String}
 * siteName
 * 
 * @author jsanca
 */
public interface Configuration extends Serializable
{
	...
	/**
	 * Returns the site configuration for the site associated to this
	 * configuration.
	 * 
	...
	/**
	 * Gets a single property, defaultValue if the property does not exists in
	...
	{
		return this.getSiteConfiguration().getString( DotSamlConstants.DOTCMS_SAML_SERVICE_PROVIDER_CUSTOM_METADATA_PATH, DotSamlConstants.DOTCMS_SAML_SERVICE_PROVIDER_CUSTOM_METADATA_PATH_DEFAULT_VALUE );
	}
	...
	{
		return this.getSiteConfiguration().getBoolean( DotSamlConstants.DOT_SAML_VERIFY_ASSERTION_SIGNATURE, true );
	}
	...
	{
		return this.getSiteConfiguration().getBoolean( DotSamlConstants.DOT_SAML_VERIFY_SIGNATURE_PROFILE, true );
	}
	...
	{
		return this.getSiteConfiguration().getBoolean( DotSamlConstants.DOT_SAML_VERIFY_SIGNATURE_CREDENTIALS, true );
	}

	/**
	 * Gets the signing credentials, usually they are pull from the idp metadata
	 * xml, the idp metadata is generated from shibboleth, for instance or any
	 * other Idp.
	 * 
	 ...
	 * URL is used on the metadata to fill out the AssertionConsumerService's
	 * 
	 * @return String
	 */
	public default String getAssertionConsumerEndpoint()
	{
		...
		return this.getSiteConfiguration().getString( DotSamlConstants.DOT_SAML_ASSERTION_CUSTOMER_ENDPOINT_URL, spIssuerValue );
	}

	/**
	 * In case the user wants some specific logout url, otherwise null. This URL
	 * is used on the metadata to fill out the assertion customer service's
	 * 
	 * @return String
	 */
	public default String getSingleLogoutEndpoint()
	{
		...
		return this.getSiteConfiguration().getString( DotSamlConstants.DOT_SAML_LOGOUT_SERVICE_ENDPOINT_URL, spIssuerValue );
	}
	...
	{
		final String className = this.getSiteConfiguration().getString( DotSamlConstants.DOT_SAML_SERVICE_PROVIDER_CUSTOM_CREDENTIAL_PROVIDER_CLASSNAME, null );
	...
	{
		final String className = this.getSiteConfiguration().getString( DotSamlConstants.DOT_SAML_ID_PROVIDER_CUSTOM_CREDENTIAL_PROVIDER_CLASSNAME, null );
