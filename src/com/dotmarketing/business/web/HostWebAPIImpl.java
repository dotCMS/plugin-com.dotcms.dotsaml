...
public class HostWebAPIImpl extends HostAPIImpl implements HostWebAPI {
    ...
    public Host getCurrentHost(final HttpServletRequest request, final User userParam)
        ...
        boolean respectAnonPerms = Boolean.FALSE;
        try {
            respectAnonPerms = user == null || !userWebAPI.isLoggedToBackend(request);
        } catch (Exception e) {
            Logger.warn(this, String.format("An error occurred when rettrieving user from the request: %s", e.getMessage()));
        }

        Host optionalHost = this.getCurrentHostFromRequest(request, user, respectAnonPerms);

        if ((null != optionalHost && UtilMethods.isSet(optionalHost.getIdentifier())) && !respectAnonPerms){
            optionalHost = this.getCurrentHostFromSession(request, user, respectAnonPerms);
        }

        final Host host = null != optionalHost && UtilMethods.isSet(optionalHost.getIdentifier()) ? optionalHost : resolveHostName(request.getServerName(),
    ...
    private void storeCurrentHost(final HttpServletRequest request, final User user, final Host host) {
        ...
        final UserWebAPI userWebAPI = WebAPILocator.getUserWebAPI();
        boolean isBackendUser = Boolean.FALSE;
        try {
            isBackendUser = userWebAPI.isLoggedToBackend(request);
        } catch (Exception e) {
            Logger.warn(this, String.format("An error occurred when rettrieving user from the request: %s", e.getMessage()));
        }
        if (session != null && isBackendUser) {
    ...
    private Host getCurrentHostFromSession(final HttpServletRequest request, final User user, final boolean respectAnonPerms)
    ...
    private Host getCurrentHostFromRequest(final HttpServletRequest request, final User user, final boolean respectAnonPerms)
            throws DotDataException, DotSecurityException {

	    final String hostId = request.getParameter("host_id");

	    if (hostId != null && !respectAnonPerms) {
	        return find(hostId, user, respectAnonPerms);
            ...
            final String hostName = request.getParameter(Host.HOST_VELOCITY_VAR_NAME);
	            ...
	            return (Host) request.getAttribute(WebKeys.CURRENT_HOST);
        	} else {
	            return null;
        	}
        }
